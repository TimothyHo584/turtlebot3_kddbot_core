...
#include "turtlebot3_kddbot_motor_driver.h"
...
{
  torque_ = false;
  dynamixel_limit_max_velocity_ = KDDBOT_DXL_LIMIT_MAX_VELOCITY;
}
...
bool Turtlebot3MotorDriver::controlMotor(const float wheel_radius, const float wheel_separation, float* value)
{
  ...
  float lin_vel = value[LEFT_FRONT];
  float ang_vel = value[RIGHT_FRONT];

  wheel_velocity_cmd[LEFT_REAR]   = lin_vel - (ang_vel * wheel_separation / 2);
  wheel_velocity_cmd[RIGHT_REAR]  = lin_vel + (ang_vel * wheel_separation / 2);
  wheel_velocity_cmd[LEFT_FRONT]   = lin_vel - (ang_vel * wheel_separation / 2);
  wheel_velocity_cmd[RIGHT_FRONT]  = lin_vel + (ang_vel * wheel_separation / 2);

  wheel_velocity_cmd[LEFT_REAR]  = constrain(wheel_velocity_cmd[LEFT_REAR]  * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  wheel_velocity_cmd[RIGHT_REAR] = constrain(wheel_velocity_cmd[RIGHT_REAR] * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  wheel_velocity_cmd[LEFT_FRONT]  = constrain(wheel_velocity_cmd[LEFT_FRONT]  * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
  wheel_velocity_cmd[RIGHT_FRONT] = constrain(wheel_velocity_cmd[RIGHT_FRONT] * VELOCITY_CONSTANT_VALUE / wheel_radius, -dynamixel_limit_max_velocity_, dynamixel_limit_max_velocity_);
