...
//###########################################################################
//This version can be use in opencr library version 1.4.15 !!!! Please check!
//###########################################################################
...
void publishSensorStateMsg(void)
{
  ...
  dxl_comm_result = motor_driver.readEncoder(sensor_state_msg.bleft_encoder, sensor_state_msg.bright_encoder, sensor_state_msg.left_encoder, sensor_state_msg.right_encoder);

  if (dxl_comm_result == true)
    updateMotorInfo(sensor_state_msg.bleft_encoder, sensor_state_msg.bright_encoder, sensor_state_msg.left_encoder, sensor_state_msg.right_encoder);
  else
    return;
...
{
  //Timothy 2021.3.10
  version_info_msg.hardware = "1.0.0";
  version_info_msg.software = "1.0.0";
...
bool calcOdometry(double diff_time)
{
  ...
  double wheel_f_l, wheel_f_r, wheel_b_l, wheel_b_r;      // rotation value of wheel [rad]
...
//Timothy 2021.3.10
//  wheel_l = wheel_r = 0.0;
  wheel_f_l = wheel_f_r = wheel_b_l = wheel_b_r = 0.0;
...
//  wheel_r = TICK2RAD * (double)last_diff_tick[RIGHT];
  ...
  wheel_b_l = TICK2RAD * (double)last_diff_tick[LEFT_REAR];
  wheel_b_r = TICK2RAD * (double)last_diff_tick[RIGHT_REAR];
  wheel_f_l = TICK2RAD * (double)last_diff_tick[LEFT_FRONT];
  wheel_f_r = TICK2RAD * (double)last_diff_tick[RIGHT_FRONT];
...
//  delta_s     = WHEEL_RADIUS * (wheel_r + wheel_l) / 2.0;
  ...
  delta_s     = WHEEL_RADIUS * (wheel_b_r + wheel_b_l + wheel_f_r + wheel_f_l) / 4.0;
...
//  last_velocity[RIGHT] = wheel_r / step_time;
  ...
  last_velocity[LEFT_REAR]  = wheel_b_l / step_time;
  last_velocity[RIGHT_REAR] = wheel_b_r / step_time;
  last_velocity[LEFT_FRONT]  = wheel_f_l / step_time;
  last_velocity[RIGHT_FRONT] = wheel_f_r / step_time;
  last_theta = theta;
...
//    saved_tick[RIGHT] = current_tick[RIGHT];
    ...
    saved_tick[RIGHT_FRONT] = current_tick[RIGHT_FRONT];

//    diff_encoder = TEST_DISTANCE / (0.207 / 4096); // (Circumference of Wheel) / (The number of tick per revolution) Kddbot Circumference : 0.464956
    diff_encoder = TEST_DISTANCE / (WHEEL_RADIUS*2*3.14159265359 / 4096);
    tTime[6] = millis();
  }
  else if (buttons & (1<<1))
  {
...
//    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / (0.207 / 4096);
    diff_encoder = (TEST_RADIAN * TURNING_RADIUS) / (WHEEL_RADIUS*2*3.14159265359 / 4096);
    tTime[6] = millis();
  }
...
//    if (abs(saved_tick[RIGHT] - current_tick[RIGHT]) <= diff_encoder)
    if (abs(saved_tick[RIGHT_REAR] - current_tick[RIGHT_REAR]) <= diff_encoder && abs(saved_tick[RIGHT_FRONT] - current_tick[RIGHT_FRONT]) <= diff_encoder)
    {
...
{
  static char *joint_states_name[] = {(char*)"wheel_bleft_joint", (char*)"wheel_bright_joint", (char*)"wheel_left_joint", (char*)"wheel_right_joint"};
